...
import {MenuOrientation} from "../../../../shared/enums/menu-orientation";
import {Store} from "@ngrx/store";
import {State as RootState} from "../../../../infraestructure/redux_store/redux-reducers";
import {Observable} from "rxjs/Rx";
import {isAuthenticated} from "../../../../infraestructure/redux_store/states/authentication/login-selectors";
...
@Component({
  ...
  styleUrls: ['./admin-layout-component.component.scss']
})
export class AdminLayoutComponentComponent implements OnInit,OnDestroy, AfterViewInit {
  layoutCompact = true;
...
@ViewChild('scrollPanel') layoutMenuScrollerViewChild: ScrollPanel;
  ...
  pageEntire$:Observable<boolean>;



  constructor(public renderer2: Renderer2, public zone: NgZone, private route: ActivatedRoute) {

    this.pageEntire$ = this.route.data.pipe(map(data => !!data.pageEntire));
  ...
  ngAfterViewInit() {
    ...
    setTimeout(() => {
      this.layoutMenuScrollerViewChild.moveBar();
       ...
       const node = document.getElementById('layout-content');

      const observer = new MutationObserver(() => {

        this.applyAjust = node.offsetHeight < window.innerHeight - 63;
